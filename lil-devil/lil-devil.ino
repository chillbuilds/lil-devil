...
const unsigned char devil [] PROGMEM = {
  ...
  0x30, 0x00, 0x00, 0x0c, 0x38, 0x07, 0xf0, 0x1c, 0x3e, 0x1f, 0xfc, 0x3c, 0x3f, 0xf8, 0x0f, 0xfc, 
  0x1f, 0xe0, 0x03, 0xfc, 0x1f, 0xc0, 0x01, 0xfc, 0x0f, 0x80, 0x00, 0xf8, 0x07, 0x1e, 0x00, 0x70, 
  ...
  0x0c, 0x00, 0x00, 0x18, 0x06, 0x00, 0x00, 0x30, 0x06, 0x10, 0x00, 0x30, 0x03, 0x0f, 0xe0, 0x60, 
  0x03, 0x80, 0x00, 0xe0, 0x01, 0xc0, 0x01, 0xc0, 0x00, 0xe0, 0x03, 0x80, 0x00, 0x78, 0x0f, 0x00, 
...
void loop() {
  ...
  if(leftButtonState == LOW && options[currentOption] == "feed" && atHome == false){
    Serial.println("left in food menu");
    delay(systemDelay);
  }
  ...
  if(rightButtonState == LOW && options[currentOption] == "feed" && atHome == false){
    Serial.println("right in food menu");
    delay(systemDelay);  
  }
...
void renderHungerStatBar(){
  display.drawBitmap(4, 22,  statBarLg, 56, 8, 1);
  if(hungerLvl >= 0){
    display.drawBitmap(7, 24,  statLgFull, 9, 4, 1);
  }else{
    display.drawBitmap(7, 24,  statLgEmpty, 9, 4, 1);
  }
  if(hungerLvl >= 1){
    display.drawBitmap(17, 24,  statLgFull, 9, 4, 1);
  }else{
    display.drawBitmap(17, 24,  statLgEmpty, 9, 4, 1);
  }
  if(hungerLvl >= 2){
    display.drawBitmap(27, 24,  statLgFull, 9, 4, 1);
  }else{
    display.drawBitmap(27, 24,  statLgEmpty, 9, 4, 1);
  }
  if(hungerLvl >= 3){
    display.drawBitmap(37, 24,  statLgFull, 9, 4, 1);
  }else{
    display.drawBitmap(37, 24,  statLgEmpty, 9, 4, 1);
  }
  if(hungerLvl >= 4){
    display.drawBitmap(47, 24,  statLgFull, 9, 4, 1);  
  }else{
